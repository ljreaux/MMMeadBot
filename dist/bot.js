"use strict";var M=Object.create;var w=Object.defineProperty;var v=Object.getOwnPropertyDescriptor;var k=Object.getOwnPropertyNames;var L=Object.getPrototypeOf,B=Object.prototype.hasOwnProperty;var A=(e,n,s,c)=>{if(n&&typeof n=="object"||typeof n=="function")for(let a of k(n))!B.call(e,a)&&a!==s&&w(e,a,{get:()=>n[a],enumerable:!(c=v(n,a))||c.enumerable});return e};var T=(e,n,s)=>(s=e!=null?M(L(e)):{},A(n||!e||!e.__esModule?w(s,"default",{value:e,enumerable:!0}):s,e));var i=require("discord.js"),C=T(require("dotenv"));var m={mead:"This is a mead recipe",test:"this is a test of updating recipes"};var R=["Beginner","10 Meads","20 Meads","25 Meads","30 Meads","40 Meads","50 Meads","75 Meads","100 Meads"],$=R.reduce((e,n)=>`${e}
${n}`),E=Object.keys(m).reduce((e,n)=>`${e}
...
- https://savannahbee.com/?gclid=CjwKCAjw-L-ZBhB4EiwA76YzOVfGxD75mvkq38gsWncycP5T7m-yKFEArPeUeRhbybkul14IHzcm-BoCsv8QAvD_BwE`},{command:"!equipmentlist",response:"Here is basic equipment list: https://discord.com/channels/568934729676488704/853838172059729920/949501378462634035"},{command:"!raptPillBt",response:"Check out this cool project: https://discord.com/channels/568934729676488704/853838172059729920/1247224944283353148"}],p="?rank ",f=e=>{let n=!1;return["Administrator","Moderator","Mazer of the Week","Patreon Bot","Discord Mead Leader","MMM Patron","Mead Bot","Rythm","Server Booster","EasyPoll","Live Countdown Bot","YouTube Member","technician","YouTube","YT Bot","UMM2024","UMM 2024 Purgatory Member"].forEach(c=>{(c.toLowerCase()===e.toLowerCase()||c.toLowerCase().includes(e.toLowerCase()))&&(n=!0)}),n};var j=e=>-668.962+1262.45*e-776.43*e**2+182.94*e**3,g=(e,n)=>{let s=-668.962+1262.45*e-776.43*e**2+182.94*e**3,c=-668.962+1262.45*n-776.43*n**2+182.94*n**3,a=.22+.001*s,o=(a*s+c)/(1+a),r=(s-o)/(2.0665-.010665*s),t=Math.round(r*(n/.794)*100)/100;return[Math.round(j(n)+4.5*t),t]};C.default.config();var{token:x,welcomeChannel:z="",botSpamChannel:H=""}=process.env,l=new i.Client({intents:[i.GatewayIntentBits.Guilds,i.GatewayIntentBits.GuildMessages,i.GatewayIntentBits.MessageContent,i.GatewayIntentBits.GuildMembers,i.GatewayIntentBits.GuildPresences]});l.login(x);l.once(i.Events.ClientReady,e=>{console.log(`Ready! Logged in as ${e.user.tag}`)});l.on("messageCreate",e=>{if(e.author.bot)return;let n=e.content,{member:s}=e,c=s?.roles.cache.filter(o=>o.name.toLowerCase().includes("mead")||o.name.toLowerCase()==="beginner");if(n.toLowerCase().startsWith(p)){let o=n.substring(p.length);if(f(o)){e.channel.send(`You are in this discord server, but we do not grant you the rank of ${o}`);return}let r=e.guild?.roles.cache.find(t=>(o==="10"&&(o="10 "),t.name.toLowerCase()===o.toLowerCase()||t.name.toLowerCase().includes(o.toLowerCase())));if(!r){e.channel.send(`The role ${o} is not a valid role.`);return}c?.forEach(t=>s?.roles.remove(t.id)),s?.roles.add(r.id),e.channel.send(`You have been assigned to role "${r.name}"`).catch(t=>console.error(t));return}if(n.toLowerCase().startsWith("!recipes")){let[,o]=n.split(" "),r=o?.toLowerCase()||"";if(o&&r in m){e.channel.send(m[r]);return}else if(o){e.channel.send(`The recipe ${o} is not a valid recipe command.`);return}}if(n.toLowerCase().startsWith("!abv")){let[,o,r]=n.split(" "),[t,d]=[Number(o),Number(r)||.996];if(isNaN(t)||isNaN(d)||t<d||t>1.22||d>1.22||t-d>.165){e.channel.send("Please enter a valid number for OG and FG. Example: !abv 1.050 1.010");return}let[b,y]=g(t,d);e.channel.send(`An OG of ${t} and an FG of ${d} will make ${y}% ABV and ${b} delle units.`);return}for(let o of h)(o.command===n||n.includes(o.command))&&e.channel.send(o.response).catch(r=>console.error(r));let a=o=>{let r=[];return r=o.map(t=>`${t.command}
`),r};if(n=="!list"){let o=`
**Available Bot Commands**
`,r=a(h),d=`${o}${r}`.replaceAll(",","");e.channel.send(d).catch(u=>console.error(u))}});l.on("guildMemberAdd",e=>{l.channels.cache.get(z).send(`Welcome to the MMM Discord Server <@${e.user.id}>!
 Please head over to <#${H}> and run **?rank (rank)** to recieve a rank and join your mini mead making community.
