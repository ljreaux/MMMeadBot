"use strict";var M=Object.create;var f=Object.defineProperty;var k=Object.getOwnPropertyDescriptor;var v=Object.getOwnPropertyNames;var B=Object.getPrototypeOf,L=Object.prototype.hasOwnProperty;var A=(e,n,s,c)=>{if(n&&typeof n=="object"||typeof n=="function")for(let i of v(n))!L.call(e,i)&&i!==s&&f(e,i,{get:()=>n[i],enumerable:!(c=k(n,i))||c.enumerable});return e};var T=(e,n,s)=>(s=e!=null?M(B(e)):{},A(n||!e||!e.__esModule?f(s,"default",{value:e,enumerable:!0}):s,e));var a=require("discord.js"),g=T(require("dotenv"));var l={mead:"This is a mead recipe",test:"this is a test of updating recipes"};var R=["Beginner","10 Meads","20 Meads","25 Meads","30 Meads","40 Meads","50 Meads","75 Meads","100 Meads"],$=R.reduce((e,n)=>`${e}
...
- https://savannahbee.com/?gclid=CjwKCAjw-L-ZBhB4EiwA76YzOVfGxD75mvkq38gsWncycP5T7m-yKFEArPeUeRhbybkul14IHzcm-BoCsv8QAvD_BwE`},{command:"!equipmentlist",response:"Here is basic equipment list: https://discord.com/channels/568934729676488704/853838172059729920/949501378462634035"},{command:"!raptPillBt",response:"Check out this cool project: https://discord.com/channels/568934729676488704/853838172059729920/1247224944283353148"}],h="?rank ",w=e=>{let n=!1;return["Administrator","Moderator","Mazer of the Week","Patreon Bot","Discord Mead Leader","MMM Patron","Mead Bot","Rythm","Server Booster","EasyPoll","Live Countdown Bot","YouTube Member","technician","YouTube","YT Bot","UMM2024","UMM 2024 Purgatory Member"].forEach(c=>{(c.toLowerCase()===e.toLowerCase()||c.toLowerCase().includes(e.toLowerCase()))&&(n=!0)}),n};var j=e=>-668.962+1262.45*e-776.43*e**2+182.94*e**3,b=(e,n)=>{let s=-668.962+1262.45*e-776.43*e**2+182.94*e**3,c=-668.962+1262.45*n-776.43*n**2+182.94*n**3,i=.22+.001*s,o=(i*s+c)/(1+i),t=(s-o)/(2.0665-.010665*s),r=Math.round(t*(n/.794)*100)/100;return[Math.round(j(n)+4.5*r),r]};g.default.config();var{token:x,welcomeChannel:P="",botSpamChannel:Y=""}=process.env,m=new a.Client({intents:[a.GatewayIntentBits.Guilds,a.GatewayIntentBits.GuildMessages,a.GatewayIntentBits.MessageContent,a.GatewayIntentBits.GuildMembers,a.GatewayIntentBits.GuildPresences]});m.login(x);m.once(a.Events.ClientReady,e=>{console.log(`Ready! Logged in as ${e.user.tag}`)});m.on("messageCreate",e=>{if(e.author.bot)return;let n=e.content,{member:s}=e;if(n.includes("?kick")){if(console.log(e.mentions.users),!e.member?.permissionsIn(e.channel.id).has(a.PermissionsBitField.Flags.Administrator))return;let[,o]=n.split(" ");if(!o){e.channel.send("You need to specify a user to kick.");return}console.log(o);let t=e.guild?.members.cache.find(r=>r.user.tag===o);if(!t){e.channel.send("User not found.");return}e.guild?.members.kick(t),e.channel.send(`${t.user.tag} has been kicked.`);return}let c=s?.roles.cache.filter(o=>o.name.toLowerCase().includes("mead")||o.name.toLowerCase()==="beginner");if(n.toLowerCase().startsWith(h)){let o=n.substring(h.length);if(w(o)){e.channel.send(`You are in this discord server, but we do not grant you the rank of ${o}`);return}let t=e.guild?.roles.cache.find(r=>(o==="10"&&(o="10 "),r.name.toLowerCase()===o.toLowerCase()||r.name.toLowerCase().includes(o.toLowerCase())));if(!t){e.channel.send(`The role ${o} is not a valid role.`);return}c?.forEach(r=>s?.roles.remove(r.id)),s?.roles.add(t.id),e.channel.send(`You have been assigned to role "${t.name}"`).catch(r=>console.error(r));return}if(n.toLowerCase().startsWith("!recipes")){let[,o]=n.split(" "),t=o?.toLowerCase()||"";if(o&&t in l){e.channel.send(l[t]);return}else if(o){e.channel.send(`The recipe ${o} is not a valid recipe command.`);return}}if(n.toLowerCase().startsWith("!abv")){let[,o,t]=n.split(" "),[r,d]=[Number(o),Number(t)||.996];if(isNaN(r)||isNaN(d)||r<d||r>1.22||d>1.22||r-d>.165){e.channel.send("Please enter a valid number for OG and FG. Example: !abv 1.050 1.010");return}let[y,C]=b(r,d);e.channel.send(`An OG of ${r} and an FG of ${d} will make ${C}% ABV and ${y} delle units.`);return}for(let o of u)(o.command===n||n.includes(o.command))&&e.channel.send(o.response).catch(t=>console.error(t));let i=o=>{let t=[];return t=o.map(r=>`${r.command}
`),t};if(n=="!list"){let o=`
**Available Bot Commands**
`,t=i(u),d=`${o}${t}`.replaceAll(",","");e.channel.send(d).catch(p=>console.error(p))}});m.on("guildMemberAdd",e=>{m.channels.cache.get(P).send(`Welcome to the MMM Discord Server <@${e.user.id}>!
